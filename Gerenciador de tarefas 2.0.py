...
class Gerenciador_Tarefas:
    ...
    def criar_widgets(self):
        ...
        ttk.Label(Entrada_frame, text="Prioridade:").grid(row=2, column=0, padx=5,sticky= tk.W)
        ...
        ttk.Label(Entrada_frame,text='Data de Vencimento').grid(row=3,column=0, padx=5, sticky=tk.W)
    ...
    def marcar_concluida(self):
        ...
        for opcao in self.tarefas:
            if opcao['id']==id_tarefa:
                opcao['concluída']= not opcao ['concluída']
                break
    ...
    def atualizar_lista(self):
        ...
        for tarefa in self.tarefas:
            item = self.tree.insert('',tk.END,values=(tarefa['id'],tarefa['tarefa'],tarefa['descricao'],tarefa['prioridade'],tarefa['data'],'sim' if tarefa['concluída'] else 'não' ))    
            if tarefa ['concluída']:
                self.tree.tag_configure('concluída', foreground ='gray')
                self.tree.item(item, tags=('concluída',))
            elif tarefa ['prioridade'] == 'Alta':
                self.tree.tag_configure('alta', foreground = 'red')
                self.tree.item(item, tags=('alta'))
            elif tarefa ['prioridade'] == 'Média':
                self.tree.tag_configure('media', foreground = 'orange')
                self.tree.item(item, tags=('media'))
            else:
                self.tree.tag_configure('baixa', foreground = 'green')
                self.tree.item(item, tags = ('baixa'))
